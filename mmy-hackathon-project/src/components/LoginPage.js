  ...
  const fetchPatientProfile = async (email) => {
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem("patientData", JSON.stringify(result));
        ...
        // If patient profile fetch fails, still navigate to dashboard
        navigate("/patient-dashboard");
      }
    } catch (error) {
      console.error("Network error while fetching patient data:", error);
      // If there's a network error, still navigate to dashboard
      navigate("/patient-dashboard");
    }
  };
