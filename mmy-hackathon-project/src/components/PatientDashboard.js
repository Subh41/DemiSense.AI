...
const PatientDashboard = () => {
    ...
    const fetchPatientData = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          localStorage.setItem("patientId", result.patientId); // Ensure this is stored
          console.log("Stored patientId:", result.patientId);
        } else {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        ...
        setAppointments(data); // Update state with the fetched appointments
      } catch (error) {
  ...
  const renderProfileImage = () => {
    ...
    return (
      ...
      >
        {/* <User Circle size={48} color="#9ca3af" /> */}
      </div>
    );
  };
