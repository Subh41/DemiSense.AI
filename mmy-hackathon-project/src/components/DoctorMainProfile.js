...
const DoctorMainProfile = () => {
  ...
  // Pull in any doctor data from the route state or localStorage
  const [doctorData, setDoctorData] = useState(
    location.state?.doctorData || {}
  );
  useEffect(() => {
    // Get doctor data from both possible sources
    const storedDoctorProfile = localStorage.getItem("doctorProfile");
    const storedDoctorData = localStorage.getItem("doctorData");
    
    try {
      if (storedDoctorProfile) {
        const parsedProfile = JSON.parse(storedDoctorProfile);
        setDoctorData(parsedProfile);
        console.log("Loaded from doctorProfile:", parsedProfile);
      } else if (storedDoctorData) {
        const parsedData = JSON.parse(storedDoctorData);
        setDoctorData(parsedData);
        console.log("Loaded from doctorData:", parsedData);
      } else if (location.state?.doctorData) {
        setDoctorData(location.state.doctorData);
        console.log("Loaded from location state:", location.state.doctorData);
      } else {
        console.log("No doctor data found in any storage location");
      }
    } catch (error) {
  ...
  // The doctor's license number for queries
  const doctorMedicalLicenseNumber = doctorData?.medicalLicenseNumber;
  // Add debug log
  console.log("Medical License Number:", doctorMedicalLicenseNumber);
  ...
  const [consultationTime, setConsultationTime] = useState("9 AM - 5 PM");
  ...
  const getStatusColor = (status) => {
    switch (status.toLowerCase()) {
      ...
      case "stable":
        return "#4CAF50"; // Green
      default:
        return "#4A90E2"; // Blue for "regular" or others
    }
  };
...
const fetchNotifications = async () => {
    try {
      if (!doctorMedicalLicenseNumber) {
        console.log("No medical license number available");
        return;
      }
  
      console.log("Fetching notifications for:", doctorMedicalLicenseNumber);
      const response = await fetch(
        `http://localhost:5000/api/notifications/${doctorMedicalLicenseNumber}`
      );
      ...
      if (data && Array.isArray(data.notifications)) {
        ...
        console.log("Setting sorted notifications:", sortedNotifications);
      } else {
        console.log("Invalid notifications format:", data);
        setNotifications([]);
      }
    } catch (error) {
  ...
  // Once we know the doctorMedicalLicenseNumber, fetch notifications
  useEffect(() => {
    if (doctorMedicalLicenseNumber) {
      console.log("Fetching notifications for license number:", doctorMedicalLicenseNumber);
      fetchNotifications();
  ...
  const NotificationList = () => {
    console.log("Rendering NotificationList, notifications:", notifications);
    return (
  ...
  // Edit consultation times
  const ConsultationEditSection = () => (
    <div style={styles.editSection}>
      <h3 style={styles.editHeader}>Edit Consultation Times</h3>
      <div style={styles.formGroup}>
        ...
        <select
          ...
          onChange={(e) => {
            ...
            setConsultationDays(selectedDays);
          }}
          style={styles.formInput}
        >
      ...
      <div style={styles.formGroup}>
        <label style={styles.formLabel}>Consultation Time</label>
        <input
          type="text"
          placeholder="Enter consultation time (e.g., 9 AM - 5 PM)"
      ...
      <div style={styles.editButtons}>
        <button
          style={styles.submitButton}
          onClick={() => {
            // Save consultation times logic here
        ...
        return (
          <div style={styles.profileContent}>
            ...
            <div style={styles.consultationSection}>
              <h3 style={styles.sectionTitle}>Consultation Availability</h3>
              <p style={styles.consultationText}>
                Available Days: {consultationDays.join(", ")}
              </p>
              <p style={styles.consultationText}>
            ...
            {showEditSection && <ConsultationEditSection />}
          </div>
        );
      case "patients":
        return (
          <div style={styles.patientsContent}>
            ...
            <div style={styles.patientsHeader}>
              ...
              <button 
                ...
                onClick={() => window.open("http://localhost:5000/patient-registration", "_blank")}
              >
          ...
          <div style={styles.prescriptionsContent}>
            ...
            <div style={styles.prescriptionFilters}>
              ...
              <button 
                ...
                onClick={() => {
                  // Check if any patient is selected from the list
                  const firstPatient = patients[0]; // For demo, select first patient
                  if (firstPatient) {
                  ...
                  <div key={index} style={styles.prescriptionCard}>
                    ...
                    <div style={styles.prescriptionCardFooter}>
                      <button 
                        ...
                        onClick={() => {
                          alert("Prescription details functionality coming soon!");
                        }}
                      >
                        View Details
                      </button>
      ...
      <div style={styles.sidebar}>
        ...
        <nav style={styles.navigation}>
          ...
          <button
            style={{
              ...styles.navButton,
              backgroundColor: showNotifications ? "#4A90E2" : "transparent",
              color: showNotifications ? "#fff" : "#333",
            }}
            onClick={() => {
...
/* -------------------- STYLES -------------------- */
const styles = {
  /* unchanged styles from your code */
  container: {
    display: "flex",
    minHeight: "100vh",
